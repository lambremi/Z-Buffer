...
def traceFacette2_test(p1, color1, p2, color2, p3, color3, m):
    # tracage des 3 segments
    x1, y1, z1 = p1
    x2, y2, z2 = p2
    x3, y3, z3 = p3
    ...
    while X3 >= X1:
        fillMatrice_test(X1,YL,ZL,np.copy(CL),YC,ZC,np.copy(CC),m)
        X1 += 1
        edyL -= dyL
        edzL -= dzL
        edcL -= dcL
...
def fillMatrice_test(X1,YL,ZL,CL,YC,ZC,CC,m):
    if(YL>YC):
        YL, YC = YC, YL
        ZL, ZC = ZC, ZL
        CL, CC = CC, CL
